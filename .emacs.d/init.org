...
Of course it comes with the basic features of a text editor, but it is much more;
...
and effort to make meaningful documentation --even though the project it private and will likely only be seen by me.
...
without any interruption to the current work flow.
...
(setq org-capture-templates
  `(
     ...
     ("e" "Notes related to Emacs (•̀ᴗ•́)و" 
     ...
     ("r" "Arbitrary ideas and notes (─‿‿─)"
...
Let's use this! The source block is replaced with the generated image in-place, by default.
...
/'Along the way, I may put pause the task for some reason then  return to it.
  This may be since I'm “Blocked” since I need something, or
  the task has been put on “hold” since it may not be important right now,
  and it may be “cancelled” eventually.  
'/
...
/' Note that we could omit the “center, left, right” if we wished, or used a “header” instead.'/
#+end_src
...
# (shell-command "rm normal_task_states.png")
